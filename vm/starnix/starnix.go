...
func (pool *Pool) Create(workdir string, index int) (vmimpl.Instance, error) {
	...
	if _, err := inst.runFfx(30*time.Second, "config", "set", "product.path", filepath.Join(inst.fuchsiaDirectory, "out/starnix/obj/products/workbench/workbench_eng.x64/product_bundle")); err != nil {
		return nil, err
	}
...
func (pool *Pool) Close() error {
	...
	// ffx daemon will exit automatically when it sees its isolation dir removed
...
func (inst *instance) startFuchsiaVM() error {
	...
	if _, err := inst.runFfx(5*time.Minute, "emu", "start", "--headless", "--name", inst.name, "--net", "user"); err != nil {
		return err
	}
	return nil
}
